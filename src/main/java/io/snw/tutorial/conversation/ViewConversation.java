...
public class ViewConversation {
    ...
    private class FinishMessage extends MessagePrompt {
        ...
        public Prompt getNextPrompt(ConversationContext context) {
            ...
            String location = ServerTutorial.getInstance().getServer().getPlayer(player.getName()).getLocation().getWorld().getName()
            		+ ","
            		+ ServerTutorial.getInstance().getServer().getPlayer(player.getName()).getLocation().getX()
            		+ ","
            		+ ServerTutorial.getInstance().getServer().getPlayer(player.getName()).getLocation().getY()
            		+ ","
            		+ ServerTutorial.getInstance().getServer().getPlayer(player.getName()).getLocation().getZ()
            		+ ","
            		+ ServerTutorial.getInstance().getServer().getPlayer(player.getName()).getLocation().getYaw()
            		+ ","
            		+ ServerTutorial.getInstance().getServer().getPlayer(player.getName()).getLocation().getPitch();
    ...
    private class Prefix implements ConversationPrefix {
        ...
        public String getPrefix(ConversationContext context) {
            return ChatColor.translateAlternateColorCodes('&', "&e&lVanquish&c&lTutorial &7&l>&r");
        }
    }
